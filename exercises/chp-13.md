...
3. db.회원.find();
4. _db.회원.find({성별: {$ne: "남"}}, {_id: 1, 성별: 1});
5. _db.회원.find({성별: "남"}).sort({나이: 1});
6. db.회원.find({_id:{$exist:true}}).count();
7. _db.회원.updateMany({나이: {$gt: 25}}, {$set: {성별: "여"}});
8. db.회원.deleteMany({});
...
키-값db  - 간단한 키-값 쌍 저장
문서db - json, bson 문서형태로 저장
컬럼패밀리db - 열 단위로 저장, 대용량 분석에 적합
그래프db - 노드-간선 구조, 복잡한 관계형 데이터 표현에 적합

**11 몽고DB와 관계형 데이터베이스를 비교 • 설명하시오.**

몽고db는 스키마리스 형태로 문서기반의 데이터 구조를 가지며 수평적 확장을 합니다. 자체 쿼리 문법을 사용합니다.
관계형 데이터베이스는 고정 스키마 형태로 테이블 기반의 데이터구조를 가지며 주로 수직 확장을 합니다. sql 쿼리언어를 사용합니다.
